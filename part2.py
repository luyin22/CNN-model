...
class NetworkLstm(tnn.Module):
    ...
    def __init__(self):
        ...
        self.ln1 = torch.nn.Linear(100, 64)
        self.rl = torch.nn.ReLU()
        self.ln2 = torch.nn.Linear(64, 1)
...
class NetworkCnn(tnn.Module):
    ...
    def __init__(self):
        ...
        Create and initialise weights and biases for the layers.
        """
        self.cv1 = torch.nn.Conv1d(50, 50, 8, padding=5)
        self.cv2 = torch.nn.Conv1d(50, 50, 8, padding=5)
        self.cv3 = torch.nn.Conv1d(50, 50, 8, padding=5)
        self.rl = torch.nn.ReLU()
        self.mp = torch.nn.MaxPool1d(4)
        self.ln = torch.nn.Linear(50, 1)
...
def measures(outputs, labels):
    ...
    """
    tp = torch.sum((outputs > 0) & (labels == 1)).item()
    fp = torch.sum((outputs > 0) & (labels == 0)).item()
